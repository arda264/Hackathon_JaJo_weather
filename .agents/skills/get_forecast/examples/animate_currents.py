"""Animate depth-averaged current speed and direction over the full forecast.

Renders one frame per timestep (subsampled via --hour-step) of a currents
forecast NetCDF file (as produced by get_current_currents_forecast.py /
get_currents_forecast.py) and assembles them into an animated GIF. The
`cmems_mod_nws_phy-cur_anfc_1.5km-2D_PT1H-i` dataset provides hourly 2D
depth-averaged currents (no vertical/depth dimension), so the full week has
144 hourly timesteps. By default every hourly timestep is rendered
(--hour-step 1); increase --hour-step to reduce render time/frame count if
needed.

Each frame has two panels: a map of current speed/direction on top, and a
timeseries of current speed at the centre grid point of the domain for the
full forecast period on the bottom, with a moving red dot marking the
current timestep.
...
Run with the `metocean` pixi environment, e.g.:
    pixi run -e metocean python animate_currents.py
"""
