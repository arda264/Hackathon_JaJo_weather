"""
Wind rose from rasterized DCSM-FM monthly output.

Reads wind u/v components from monthly NetCDF files, selects the nearest grid
point to a specified lon/lat, computes meteorological wind direction (FROM),
and draws a 16-sector rose coloured by speed class using windrose.

Meteorological convention: direction the wind blows FROM.
  dir = (degrees(arctan2(-u, -v)) + 360) % 360

Run with:
  pixi run --environment metocean python output/scripts/wind_rose.py
"""
...
CALM_THRESH = 0.5       # m/s — calm threshold
...
# ---------------------------------------------------------------------------
# 2. Compute meteorological direction (FROM) and speed
# ---------------------------------------------------------------------------
...
ax.bar(direction, speed, normed=True, opening=0.9,
       ...
       cmap=plt.cm.YlOrRd, edgecolor="white", linewidth=0.4)

ax.set_legend(title=f"Wind speed ({SPEED_UNIT})", loc="lower left",
              bbox_to_anchor=(-0.15, -0.15), fontsize=8)

ax.set_title(
    f"Wind rose  (direction FROM)\n"
    f"lon={actual_lon:.2f}°E, lat={actual_lat:.2f}°N  |  "
    f"N={len(speed)}  |  calm={calm_pct:.1f}% (<{CALM_THRESH} {SPEED_UNIT})",
    fontsize=10, pad=14,
)

figfile = os.path.join(OUTDIR, "wind_rose.png")
